...
# Quantization
def quantization(dct_img,Q = Q_1):
...
# Zigzag
def zig_zag(input_matrix, block_size = 8):
    z = []
    index = -1
    bound = 0
...
# Inverse zigzag
def zig_zag_reverse(input_matrix, block_size = 8):
    ...
    output_matrix = np.tolist(output_matrix)    
    return output_matrix

# Inverse quantization
def i_quantization(q_img, Q = Q_1):
    w,h = np.shape(q_img)
    dct_img = np.zeros_like(q_img)
    h,w = [int(h/8), int(w/8)]

    for row in range(h):
        for col in range(w):
            cur_block = dct_img[row*8:(row+1)*8, col*8:(col+1)*8]
            dct_block = np.zeros_like(cur_block)
            for i in range(8):
                for j in range(8):
                    dct_block[i,j] = cur_block[i,j] * Q[i,j]

            dct_img[row*8:(row+1)*8, col*8:(col+1)*8] = dct_block
...
if __name__ == '__main__':
    dct_img = dct_cv2(img)
    q_img = quantization(dct_img, Q = Q_2)
    ...
    for row in range(h):
        for col in range(w):
            cur_block = q_img[row*8:(row+1)*8, col*8:(col+1)*8]
            tmp = zig_zag(cur_block)
            z.append(tmp)
